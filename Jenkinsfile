...
pipeline {
    ...
    stages {
                    ...
                    script {
                        try {
                            sh '''
                                ...
                                docker-compose run --entrypoint /dev-ui/build.sh offline-ui
        ...
        stage('Build reference-ui') {
            ...
            steps {
                sh "docker tag openlmis/offline-ui:latest openlmis/offline-ui:${VERSION}"
                sh "docker push openlmis/offline-ui:${VERSION}"
                    ...
                    withCredentials([string(credentialsId: 'SONAR_LOGIN', variable: 'SONAR_LOGIN'), string(credentialsId: 'SONAR_PASSWORD', variable: 'SONAR_PASSWORD')]) {
                        script {
                            sh '''
                                ...
                                docker-compose run --entrypoint ./sonar.sh offline-ui
                                docker-compose down --volumes
